...
<!-- VARIANT 06: Conditional message inside loop (fires only for Price > 50) -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="Order_Items">
        ...
        <xsl:if test="number(./Price) &gt; 50">
            <xsl:message>Test 6: High-value item detected — <xsl:value-of select="./Name"/> @ <xsl:value-of select="./Price"/></xsl:message>
        </xsl:if>
