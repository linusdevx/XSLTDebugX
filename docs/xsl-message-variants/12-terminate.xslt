...
<!-- VARIANT 12: terminate="yes" — halts transform if quantity is 0 or negative -->
<!-- To trigger this halt path, set property quantity=0 -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <xsl:if test="number($quantity) &lt;= 0">
            <xsl:message terminate="yes">Test 12: FATAL — quantity must be greater than 0 (got <xsl:value-of select="$quantity"/>)</xsl:message>
        </xsl:if>
